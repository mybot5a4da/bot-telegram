...
async def delete_panel_user(username: str) -> bool:
    ...
    async with httpx.AsyncClient(timeout=30.0, verify=True, follow_redirects=True) as client:
        token = await _get_token(client)
        headers = _auth_headers(token)
        r = await client.delete(f"{base}/api/user/{username}", headers=headers)
        if r.status_code in (200, 204, 404):
            return True
        r2 = await client.post(f"{base}/api/user/{username}/delete", headers=headers)
        if r2.status_code in (200, 204, 404):
            return True
        logger.warning("delete user %s failed: %s %s", username, r.status_code, r.text[:200])
        return False
...
def is_panel_user_exhausted(user_data: dict[str, Any] | None, local_expire_at: int | None = None) -> tuple[bool, str]:
    ...
    now = int(time.time())
    if local_expire_at and int(local_expire_at) > 0 and now >= int(local_expire_at):
        return True, "زمان"
    if not user_data:
        return False, ""
    exp = user_data.get("expire") or user_data.get("expire_at") or user_data.get("expired_at")
    try:
    ...
    if status == "expired":
        return True, "زمان"
    if status in ("limited", "disabled"):
        return True, "حجم" if status == "limited" else "زمان"
    data_limit = user_data.get("data_limit")
    used = user_data.get("used_traffic")
    if used is None:
        used = user_data.get("lifetime_used_traffic")
    try:
        if data_limit is not None and int(data_limit) > 0 and used is not None and int(used) >= int(data_limit):
            return True, "حجم"
    except (TypeError, ValueError):
        pass
    return False, ""
