...
async def run_free_test_cleanup_once() -> int:
    """یک دور پاک‌سازی تست‌های تمام‌شده. تعداد پاک‌شده را برمی‌گرداند."""
    if not config.is_panel_auto_enabled():
        return 0
    import panel as pg_panel
    ...
    for row in rows or []:
        try:
            ...
            if info is None and force_time:
                deleted = True
            ...
            lines = [
                ...
                f"🔗 @{tg_user}",
                f"🔑 <code>{uname}</code>",
                f"حذف از پنل: {'✅' if deleted else '⚠️ ناموفق — دستی از پنل پاک کنید'}",
            ]
            text_msg = "\n".join(lines)
            for admin_id in config.ADMIN_IDS:
                try:
...
async def free_test_cleanup_loop() -> None:
    """پاک‌سازی دوره‌ای تست‌های تمام‌شده."""
    ...
    while True:
        try:
            ...
            if n:
                logging.info("free test cleanup removed %s accounts", n)
        except Exception:
...
async def admin_cleanup_tests_cmd(message: Message):
    if message.from_user.id not in config.ADMIN_IDS:
        return
    await message.answer("🧹 در حال بررسی و پاک‌سازی تست‌های تمام‌شده...")
    try:
        n = await run_free_test_cleanup_once()
        await message.answer(f"✅ پاک‌سازی انجام شد. تعداد: {n}")
    except Exception as e:
