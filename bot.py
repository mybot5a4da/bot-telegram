...
async def free_test_cleanup_loop() -> None:
    """هر چند دقیقه تست‌های تمام‌شده (حجم/زمان) را از پنل پاک و به ادمین اطلاع می‌دهد."""
        ...
        try:
            ...
            for row in rows or []:
                ...
                if not uname:
                    continue
                uid = row["user_id"]
                local_exp = row["expire_at"] if "expire_at" in row.keys() else None
                try:
                ...
                try:
                    ...
                    logging.exception("cleanup get user %s", uname)
                    continue
                if info is None and local_exp and int(time.time()) >= local_exp:
                    await db.mark_free_test_expired(uid, "زمان")
                    continue
                done, reason = pg_panel.is_panel_user_exhausted(info, local_exp)
                if not done:
                    continue
                deleted = await pg_panel.delete_panel_user(uname)
                await db.mark_free_test_expired(uid, reason)
                ...
                text_msg = (
                    "🧹 تست رایگان تمام شد و پاک شد" + chr(10)
                    + f"📦 دلیل: <b>{reason}</b>" + chr(10)
                    + f"🎮 نوع: {kind}" + chr(10)
                    + f"👤 {full_name}" + chr(10)
                    + f"🆔 <code>{uid}</code>" + chr(10)
                    + f"🔗 @{tg_user}" + chr(10)
                    + f"🔑 <code>{uname}</code>" + chr(10)
                    + f"حذف از پنل: {'✅' if deleted else '⚠️ ناموفق'}"
                )
                for admin_id in config.ADMIN_IDS:
                    try:
                        await bot.send_message(admin_id, text_msg, parse_mode="HTML")
                    except Exception as e:
                        logging.warning("cleanup notify admin: %s", e)
        except Exception:
